...
interface GetOrgOauth2AppInput {
  client?: Client
  client_id: Uuid
}

type GetOrgOauth2AppReturn = OAuth2AppResponse
 ...
 * @returns {Promise<GetOrgOauth2AppReturn>} successful operation
 *
 * Possible return types: OAuth2AppResponse
 */
export default async function get_org_oauth2_app({
  client,
  client_id,
}: GetOrgOauth2AppInput): Promise<GetOrgOauth2AppReturn> {
  ...
  const result = (await response.json()) as GetOrgOauth2AppReturn
  return result
}
