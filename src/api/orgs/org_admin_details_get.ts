 ...
 */
export default async function org_admin_details_get({
  client,
  id,
}: OrgAdminDetailsGetInput): Promise<OrgAdminDetailsGetReturn> {
  const path = `/orgs/${id}/admin/details`
  const qs = buildQuery({})
  const url = path + qs
  ...
  const result = (await response.json()) as OrgAdminDetailsGetReturn
  return result
}
