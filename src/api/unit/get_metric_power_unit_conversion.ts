...
import {
  UnitMetricPowerConversion_type,
  Error_type,
  UnitMetricPower_type,
} from '../../models.js';
import { Client } from '../../client.js';

interface Get_metric_power_unit_conversion_params {
  client?: Client;
...
type Get_metric_power_unit_conversion_return =
  | UnitMetricPowerConversion_type
  | Error_type;

export default async function get_metric_power_unit_conversion({
  client,
  output_format,
  src_format,
  value,
  ...
  const result =
    (await response.json()) as Get_metric_power_unit_conversion_return;
  return result;
}
