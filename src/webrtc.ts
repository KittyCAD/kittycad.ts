...
export class WebRTC extends EventTarget {
  ...
  private pingIntervalId = setInterval(() => {
    if (this.zooModelingCommandsWs.readyState !== WebSocket.OPEN) { return }
  ...
  constructor(args: { client: Client } & Parameters<typeof ModelingCommandsWs.urlConstructFrom> ) {
    super()
    ...
    this.rtcPeerConnection.addEventListener('track', this.webRTCOnTrack.bind(this))
    this.rtcPeerConnection.addEventListener('datachannel', this.webRTCOnDataChannel.bind(this))
    this.rtcPeerConnection.addEventListener('connectionstatechange', this.webRTCOnConnectionStateChange.bind(this))
  }

  deconstructor() {
    this.deice(this.zooModelingCommandsWs)
    
    this.rtcPeerConnection.removeEventListener('track', this.webRTCOnTrack.bind(this))
    this.rtcPeerConnection.removeEventListener('datachannel', this.webRTCOnDataChannel.bind(this))
    this.rtcPeerConnection.removeEventListener('connectionstatechange', this.webRTCOnConnectionStateChange.bind(this))
  ...
  async iceOnIceServerInfo(msg: Extract<OkWebSocketResponseData, { type: 'ice_server_info' }>) {
    ...
    this.zooModelingCommandsWs.send(JSON.stringify({
      type: 'sdp_offer',
  ...
  async iceOnSdpAnswer(msg: Extract<OkWebSocketResponseData, { type: 'sdp_answer' }>) {
    // This will kick off `trickle_ice`, since it's the final piece in knowing
    // which compatible candidates to serve us.
     await this.rtcPeerConnection.setRemoteDescription(msg.data.answer as RTCSessionDescription)
  }
  ...
  async iceOnTrickleIce(msg: Extract<OkWebSocketResponseData, { type: 'trickle_ice' }>) {
  ...
  iceOnMessage(ev: MessageEvent) {
    ...
    if (!('resp' in msg)) { return /* todo: emit error */ }
  ...
  ice(ws: WebSocket) {
    ...
    this.rtcPeerConnection.addEventListener('icecandidate', this.iceOnIceCandidate.bind(this))
  }
  ...
  deice(ws: WebSocket) {
    ...
    this.rtcPeerConnection.removeEventListener('icecandidate', this.iceOnIceCandidate)
  }
  ...
  addMouseEvents(el: HTMLVideoElement) {
    ...
    const onPointerState = (targetPointerState: PointerState) => (ev: PointerEvent) => {
